...
import "./Profile.css";
import FileContainer from "./Projects/FilesModal";
...
const ProfileView = () => {
  ...
  let urlParameter2 = urlParameter.slice("=",0)

  let [uuid, setIDRequested] = useState(JSON.stringify(urlParameter2[1]));
  ...
  const getProjects = async () => {
    ...
    const { data } = await axios.get(
      `https://brogrammers-backend.herokuapp.com/project/id/?uuid=${uuid}`
    );
  ...
  console.log('uisdhfcufnv' ,JSON.stringify(uuid))
    ...
    db.collection(`users`)
      ...
      .then((querySnapshot) => {
        querySnapshot.docs.forEach((doc) => {
          ...
          console.log("idcjksdvnj", userdetails);
        });
        setinb(Inboc);
